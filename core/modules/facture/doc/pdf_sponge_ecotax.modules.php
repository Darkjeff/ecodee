 ...
 */
class pdf_sponge_ecotax extends ModelePDFFactures
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "sponge_ecotax";
		$this->description = $langs->trans('PDFSpongeDescription'). '-'.$langs->trans('Ecotax');
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT)) {
			...
			if (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL) && $tvaisnull) {
				...
				if (! empty($conf->global->WEEE_PRODUCT_ID)) {
					$product_eco_tax_id = $conf->global->WEEE_PRODUCT_ID;
					foreach ($object->lines as $line) {
						if ($line->fk_product == $product_eco_tax_id) {
					...
					if (!empty($total_eco_tax)) {
						...
						$pdf->SetTextColor(0, 0, 60);
						$pdf->SetFillColor(248, 248, 248);
						$pdf->MultiCell($col2x - $col1x, $tab2_hl, 'Dont éco-participation DEEE:', $useborder, 'L', 1);

						$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
						$pdf->MultiCell($largcol2, $tab2_hl, price($total_eco_tax, 0, $outputlangs), $useborder, 'R', 1);
					}
				}
