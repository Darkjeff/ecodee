		...
		foreach ($lines as $key => $line) {
			...
			if ($line->special_code != 1 && $line->special_code != 2) {	// Discard shipping line and ecotax lines
				...
				if (! empty($tmpproduct->array_options['options_ecotaxdeee']) && $line->qty) {
					...
					$ecolabel[$ecocateg] = dol_concatdesc($ecolabel[$ecocateg],$tmpproduct->ref.'-'.$tmpproduct->label.': '.$line->qty.' * '.price(($tmpproduct->array_options['options_ecotaxdeee'])));
				}
			}
		}
		...
		foreach ($ecoamount as $ecocateg => $value) {
			if (is_object($tmpecotaxline[$ecocateg]) && $idlineecotax[$ecocateg] > 0) {	// If ecotax line already exists for ecocateg
				if ($ecoamount[$ecocateg]) {
					...
					$tmpecotaxline[$ecocateg]->desc=$ecolabel[$ecocateg];
				...
				if ($parentobject->table_element == 'propal')   {
					$result=$parentobject->addline($desc.$ecolabel[$ecocateg], $ecoamount[$ecocateg], 1, $txtva, 0, 0, $product_id, 0, 'HT', 0, 0, 1, $rang, $special_code, '', 0, $ecoamount[$ecocateg], null, '', '', 0);
				}
				...
				if ($parentobject->table_element == 'commande') {
					$result=$parentobject->addline($desc.$ecolabel[$ecocateg], $ecoamount[$ecocateg], 1, $txtva, 0, 0, $product_id, 0, 0, 0, 'HT', '', '', '', 1, $rang, $special_code, '', 0, $ecoamount[$ecocateg], null, 0, 0);
				}
				...
				if ($parentobject->table_element == 'facture')  {
					$result=$parentobject->addline($desc.$ecolabel[$ecocateg], $ecoamount[$ecocateg], 1, $txtva, 0, 0, $product_id, 0, '', '', 0, 0, '', 'HT', 0, 1, $rang, $special_code, '', 0, 0, null, $ecoamount[$ecocateg], '', 0);
				}
